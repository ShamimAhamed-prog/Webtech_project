...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(isset($_POST['req_accept_button'])){
    $reqID = $_POST['req_accept_button'];
    $sql = "UPDATE pack_request SET agent_id='$userID' WHERE req_id=$reqID";
  ...
  $sqlPrs = "select * from pack_request WHERE agent_id=0 LIMIT $i, $j;";
  ...
  */
  ...
  $sqlTotalRows = "select count(*) as total_rows from pack_request WHERE agent_id=0;";
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
          <table>
            <tr class="top_section_head" align="center">
              <td class="td" colspan="4">Package Requests</td>
            </tr>
          </table>
        </form>
      </div>
          ...
          <tr>
            ...
            <?php while($row = mysqli_fetch_assoc($resultPurchase)){ ?>
              <tr>
                ...
                <td align="right"> <button type="submit" name="req_accept_button" class="package_button" value="<?php echo $row['req_id']; ?>">Accept</button> </td>
              </tr>
            <?php } ?>
        </table>
